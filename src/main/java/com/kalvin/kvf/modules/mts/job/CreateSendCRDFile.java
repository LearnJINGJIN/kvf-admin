 ...
 */
@Slf4j
public class CreateSendCRDFile {
    ...
    public void doWork(JobExecutionContext context, int dirNum) throws JobExecutionException {
        ...
        try{
            ...
            service.genCrdCRDTFile(date, pageNum, uploadDate, dirNum);//生成提现贷记文件
            service.genCrdDbitFile(date, pageNum, uploadDate, dirNum);//生成提现借记文件
            service.genCrdOtherFile(date, pageNum, uploadDate, dirNum);//生成提现其它文件
    ...
    private void uploadFile(String uploadDate, SFTPUtil sftp) throws Exception {
        ...
        for(int i = 0 ; i < length ; i ++){
            ...
            if(dirName.startsWith(crdCrtlType)){//提现文件目录
                uploadCrdFile(uploadDate, sftp, dirName);
            }
        }
    }
     ...
     *
     */
    private void uploadCrdFile(String uploadDate, SFTPUtil sftp, String dirName) throws Exception {
        ...
        int length = files.length;
        String crdType = CommonConstant.CRD_APP_TYPE;
        ...
        for(int i = 0 ; i < length ; i++){
            String name = files[i];
            if(name.startsWith(crdType) && !(name.startsWith(String.format("%s%s", crdType,CommonConstant.CRD_CONTROL_FILE_TYPE)))){
                typeFiles.getFileNameList().add(name);
            }
        }
        ...
        String controlFile = service.createFile(controlType, CommonConstant.CRD_APP_TYPE, CommonConstant.CRD_CONTROL_FILE_TYPE, dirName, currPath);
        sftp.mkdir(CommonUtil.getKeyValue( CommonConstant.WHJ_ACCEPT_CRD_FILE_KEY), dirName);
        sftp.putFile(String.format("%s%s", CommonUtil.getKeyValue( CommonConstant.WHJ_ACCEPT_CRD_FILE_KEY), dirName), new File(controlFile));

        /***********************************************提现文件 beg********************************************/
        int size = typeFiles.getFileNameList().size();
        for(int i = 0 ; i < size ; i ++){
            String fileName = typeFiles.getFileNameList().get(i);
            String file = String.format("%s%s%s",currPath, File.separator,fileName);
            sftp.putFile(String.format("%s%s", CommonUtil.getKeyValue( CommonConstant.WHJ_ACCEPT_CRD_FILE_KEY), dirName), new File(file));
        }
        /***********************************************提现文件 end********************************************/
