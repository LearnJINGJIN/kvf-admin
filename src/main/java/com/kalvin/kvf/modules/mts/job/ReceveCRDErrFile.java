...
//        String uploadDate = (String)context.getAttribute("upload_date");
        try{
            ...
            while(true){
                if(!CommonServiceUtil.checkHasToken( CommonConstant.WHJ_SEND_ERR_TTCRD_KEY)){//检查是否存在Token.lock文件,若没有跳出循环
                    break;
                }
                Thread.sleep(6000);
            ...
            List<String> errFiles = sftp.getRemoteFiles(CommonUtil.getKeyValue( CommonConstant.WHJ_SEND_ERR_TTCRD_KEY));
            if(null == errFiles || errFiles.size() == 0){
                return null;
            }
            sftp.putFile(CommonUtil.getKeyValue( CommonConstant.WHJ_SEND_ERR_TTCRD_KEY), token);//上传token.lock文件
                ...
                for(int i = 0 ; i < length ; i++){
                    ...
                    if(errFiles.contains(filename)){
                        ...
                        String remoteDir = String.format("%s%s%s",
                                CommonUtil.getKeyValue( CommonConstant.WHJ_SEND_ERR_TTCRD_KEY), File.separator, dir);
                        List<String> files = sftp.getRemoteFiles(remoteDir);
                        if(null != files){
                            impFiles = new ArrayList<String>();
                            for(String name : files){
                                if( !name.startsWith(String.format("%s%s",  CommonConstant.CRD_APP_TYPE ,CommonConstant.CRD_CONTROL_FILE_TYPE))
                                        && name.startsWith( CommonConstant.CRD_APP_TYPE )){
            ...
            sftp.delete(CommonUtil.getKeyValue( CommonConstant.WHJ_SEND_ERR_TTCRD_KEY), CommonConstant.TOKEN_FILE_NAME);
            return map;
