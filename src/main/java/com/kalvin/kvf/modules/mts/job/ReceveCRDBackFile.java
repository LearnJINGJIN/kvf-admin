 ...
 */
@Slf4j
public class ReceveCRDBackFile {
    ...
    public List<String> doWork(String uploadDate) throws JobExecutionException {
        ...
        try{
            ...
            while(true){
                if(!CommonServiceUtil.checkHasToken( CommonConstant.WHJ_SEND_ERR_CRD_FILE_KEY)){//检查是否存在Token.lock文件,若没有跳出循环
                    break;
                }
                Thread.sleep(6000);
            ...
            List<String> errFiles = sftp.getRemoteFiles(CommonUtil.getKeyValue( CommonConstant.WHJ_SEND_ERR_CRD_FILE_KEY));
            if(null == errFiles || errFiles.size() == 0){
                return null;
            }
            sftp.putFile(CommonUtil.getKeyValue( CommonConstant.WHJ_SEND_ERR_CRD_FILE_KEY), token);//上传token.lock文件
                ...
                for(int i = 0 ; i < length ; i++){
                    ...
                    if(errFiles.contains(String.format("%s%s", filename,  CommonUtil.getKeyValue(CommonConstant.WHJ_SEND_ERR_FILE_LAST_THREE)))){
                        ...
                        String remoteDir = String.format("%s%s%s",
                                CommonUtil.getKeyValue( CommonConstant.WHJ_SEND_ERR_CRD_FILE_KEY), File.separator, dir);
                        List<String> files = sftp.getRemoteFiles(remoteDir);
                        if(null != files){
                            for(String name : files){
                                if(name.startsWith( CommonConstant.CRD_APP_TYPE )){
            ...
            sftp.delete(CommonUtil.getKeyValue( CommonConstant.WHJ_SEND_ERR_CRD_FILE_KEY), CommonConstant.TOKEN_FILE_NAME);
            return impFiles;
