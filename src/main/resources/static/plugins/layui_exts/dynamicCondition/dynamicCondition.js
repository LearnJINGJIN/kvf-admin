...
/**
 ...
 @Author：xpl
 @version:  4.0.9 2020-12-09 修复自动选择默认选项问题。首选li中定义ops中第一个选项，没定义ops则，选config.opsList的第一个选项
    ...
    var createInstance = function(_config){
    	...
    	instance.setCondition = function(conditionArr){
    		for(var i=0;i<conditionArr.length;i++){
    			...
    			conditionTemp[1] = conditionTemp[1] || item.firstOp;
    	...
    	,create:function(config){
    		...
    		if(config.counts){
    			for(var i=0;i<config.counts && i<instance.showItems.length;i++){
    				...
    				if(ops){
    					var idx = ops.indexOf(",");
    					_item.firstOp = idx == -1 ? ops : ops.substring(0,idx);
    				}else{
    					_item.firstOp = config.opsList[0];
    				}
    				conditionObj.push(_item.firstOp);
